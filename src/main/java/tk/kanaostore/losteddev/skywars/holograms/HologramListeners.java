...
public class HologramListeners implements Listener {
  ...
  public HologramListeners() {
    this.plugin = Holograms.getPlugin();
  }
  ...
  public void onWorldLoad(WorldLoadEvent evt) {
    for (ChunkCoord coord : toRespawn.keys()) {
      if (coord.world.equals(evt.getWorld().getName())
          && evt.getWorld().isChunkLoaded(coord.x, coord.z)) {
  ...
  public void onWorldUnload(WorldUnloadEvent evt) {
    for (Hologram hologram : Holograms.listHolograms()) {
      if (hologram != null && hologram.isSpawned()
          && hologram.getLocation().getWorld().equals(evt.getWorld())) {
        if (evt.isCancelled()) {
          for (ChunkCoord coord : toRespawn.keys()) {
            if (coord.world.equals(evt.getWorld().getName())) {
              respawnAllFromCoord(coord);
  ...
  private ChunkCoord toCoord(Location location) {
    return new ChunkCoord(location.getWorld().getName(), location.getBlockX() >> 4,
