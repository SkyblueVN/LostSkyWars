...
public class PagedMenu {
  ...
  public String previousStack = "ARROW : 1 : display=&aPage {page}", nextStack = "ARROW : 1 : display=&aPage {page}";
  ...
  public PagedMenu(String name, int rows) {
    this.rows = rows > 6 ? 6 : rows < 1 ? 1 : rows;
    this.name = name;
  }
  ...
  public void setItems(List<ItemStack> items) {
    ...
    this.menus.forEach(
        menu -> menu.getInventory().getViewers().forEach(player -> player.closeInventory()));
    this.menus.clear();
    ...
    for (int i = 0; i < splitted.size(); i++) {
      ...
      if (splitted.size() > 1) {
        if (i > 0 && previousPage != -1) {
          menu.setItem(previousPage, BukkitUtils.deserializeItemStack(
              previousStack.replace("{page}", String.valueOf(i))));
        }
        if (i + 1 != splitted.size() && nextPage != -1) {
          menu.setItem(nextPage, BukkitUtils.deserializeItemStack(
              nextStack.replace("{page}", String.valueOf(i + 2))));
        }
      }
      this.menus.add(menu);
